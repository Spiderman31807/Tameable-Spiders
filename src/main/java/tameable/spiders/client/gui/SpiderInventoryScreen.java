...
public class SpiderInventoryScreen extends AbstractContainerScreen<SpiderInventoryMenu> {
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int gx, int gy) {
		...
		for(int idx = 0; idx <= maxIdx; ++idx) {
			if(menu.get().get(idx) instanceof Slot customSlot)
	...
	public ResourceLocation getUtilityTexture(Utility utility) {
		...
		String texture = switch(utility) {
			default -> "";
			case Utility.SADDLE -> "saddle_outline";
			case Utility.ARMOR -> "armor_outline";
			case Utility.SILK -> "string_outline";
			case Utility.VENOM -> "vemon" + VenomLevel;
		};
	...
	public boolean keyPressed(int key, int b, int c) {
		if(key == 256) {
			...
			PacketDistributor.sendToServer(new SpiderInventoryButtonMessage(0, x, y, z));
