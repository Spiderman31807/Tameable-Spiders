...
public abstract class ModdedSpider extends RideableSpider {
    ...
    public ResourceLocation getDefaultLootTable() {
    	if(this.entityData.get(HasRespawned) == true)
    		return EntityType.ITEM.getDefaultLootTable();
    ...
    public void baseTick() {
    	...
    	if(this.canProduceVemon()) {
    		if(this.getVenomTimer() > 0) {
    			this.setVenomTimer(this.getVenomTimer() - 1);
    		} else if(VenomSlot != -1) {
    			if(inventory.getStackInSlot(VenomSlot).getItem() == Items.GLASS_BOTTLE)
    			{
    	...
    	if(this.canProduceSilk()) {
    		if(this.getSilkTimer() > 0) {
				...
				addItem(new ItemStack(TameableSpidersModItems.SILK.get()), 0, 9, true, null);
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        if(compound.contains("VenomTimer"))
    ...
    public void harvestVemon(Player player, ItemStack bottle) {
    	bottle.shrink(1);
		player.getInventory().add(new ItemStack(TameableSpidersModItems.VENOM.get()));
    ...
    public boolean canProduceVemon() {
		...
		return getVariant().canProduceVemon(this.level());
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if(this.canProduceVemon() && this.getVemonLevel() == 9 && item == Items.GLASS_BOTTLE) {
			this.harvestVemon(player, itemstack);
