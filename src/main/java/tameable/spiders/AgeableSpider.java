...
public abstract class AgeableSpider extends Spider {
	...
	public EntityDimensions getDimensions(Pose pose) {
		return super.getDimensions(pose).scale(this.getHitboxScale());
	}
    ...
    public boolean popItem(ItemStack stack, int pickupDelay) {
	...
	protected float getStandingEyeHeight(Pose pose, EntityDimensions dimensions) {
		float Height = super.getStandingEyeHeight(pose, dimensions) * this.getModelScale();
		return isBaby() ? Height * 0.5f : Height;
   	}
    ...
    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor accessor, DifficultyInstance difficulty, MobSpawnType type, @Nullable SpawnGroupData spawnData, CompoundTag compound) {
        if (spawnData == null) {
        ...
        return super.finalizeSpawn(accessor, difficulty, type, spawnData, compound);
    }
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(IsBaby, false);
        this.entityData.define(UtilityTop_Slot, -1);
        this.entityData.define(UtilityMiddle_Slot, -1);
        this.entityData.define(UtilityBottom_Slot, -1);
        this.entityData.define(UtilityTop_Type, 0);
        this.entityData.define(UtilityMiddle_Type, 0);
        this.entityData.define(UtilityBottom_Type, 0);
	}

    @Override
    public boolean doHurtTarget(Entity entity) {
    	if(!this.getVariant().canProduceVemon(this.level()))
    ...
    protected void setupUtilities() {
		...
		if(getVariant().canProduceVemon(this.level()))
    ...
    public Utility getUtility(int type) {
    	type = Math.min(2, Math.max(type, 0));
    	return switch(type) {
