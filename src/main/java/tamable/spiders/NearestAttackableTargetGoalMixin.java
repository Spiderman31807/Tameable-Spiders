...
public abstract class NearestAttackableTargetGoalMixin extends TargetGoal {
	...
	private void constructor(Mob mob, Class<LivingEntity> targetType, int randomInterval, boolean sight, boolean reach, Predicate<LivingEntity> selector, CallbackInfo info) {
		if (selector == null)
			return;
		this.targetConditions = TargetingConditions.forCombat().range(this.getFollowDistance()).selector((entity) -> selector.test(entity) && !(entity instanceof TamableSpider spider && spider.isTame()));
	}
}