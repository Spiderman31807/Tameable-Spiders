...
public interface TamableSpider extends OwnableEntity, Saddleable {
    ...
    default boolean setEntityOnBack(ServerPlayer player) {
        CompoundTag compound = new CompoundTag();
        compound.putString("id", this.spider().getEncodeId());
        this.spider().saveWithoutId(compound);
        if (player.setEntityOnShoulder(compound)) {
            this.spider().discard();
            return true;
        } else {
            return false;
    ...
    default boolean canSitOnBack() {
        return this.canFitOnBack() && this.isTame();
    }

	default boolean canFitOnBack() {
		return false;
	}
	...
	default void usePlayerItem(Player player, InteractionHand hand, ItemStack stack) {
		...
		if (remainder != null) {
			ItemStack newStack = remainder.convertIntoRemainder(stack, current, player.hasInfiniteMaterials(), player::handleExtraItemsCreatedOnUse);
