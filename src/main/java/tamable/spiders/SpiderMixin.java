...
public abstract class SpiderMixin extends Monster implements TamableSpider {
	...
	@Override
	protected void dropCustomDeathLoot(ServerLevel server, DamageSource source, boolean playerKiller) {
		super.dropCustomDeathLoot(server, source, playerKiller);
	...
	public float getHeadRollAngle(float partialTick) {
		return Mth.lerp(partialTick % 1, this.interestedAngleO, this.interestedAngle) * 0.15f * (float) Math.PI;
	}
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if (this.isTame()) {
			...
			if (this.isOwnedBy(player)) {
				...
				if (this.isInSittingPose() && !armorStack.isEmpty() && armorStack.isDamaged() && (armorStack.getItem() instanceof SpiderArmorItem armor && armor.getMaterial().value().repairIngredient().get().test(stack))) {
					stack.shrink(1);
	...
	protected void actuallyHurt(DamageSource source, float damage) {
		...
		if (!this.canArmorAbsorb(source))
            ...
            this.getArmorItem().hurtAndBreak(Mth.ceil(damage), this, EquipmentSlot.BODY);
	}
	...
	private void registerGoals(CallbackInfo callback) {
		...
		manager.insertGoal(1, true, new AttackTargetInWebGoal(this, 1, true));
